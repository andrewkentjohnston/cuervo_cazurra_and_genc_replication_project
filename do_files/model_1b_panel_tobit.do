* Description: Cuervo-Cazurra and Genc (2008) replication - model 1b (updated data version)
...
egen row_missing = rowmiss(prop_emne voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity colonial_link)
...
xttobit prop_emne voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity colonial_link, ll(0) ul(100) re

* Store the results of the random effects model
estimates store re_model

* Export Tobit results to CSV (Excel-compatible)
esttab re_model using "../results/model_1b_results.csv", replace title("Random Effects Panel Tobit Results") label se b(%9.4f) star(* 0.10 ** 0.05 *** 0.01)

log close

* Estimating a pooled Tobit model
log using "../results/model_1b_replication.log", append text
tobit prop_emne voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity colonial_link, ll(0) ul(100)
...
asdoc pwcorr prop_emne voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity colonial_link, save(../results/model_1b_correlation_matrix.doc) replace title(Correlation Matrix)
