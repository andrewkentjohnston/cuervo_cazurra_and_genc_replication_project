...
xttobit prop_emne_capital_excl_col_link voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity, ll(0) ul(100) re

* Store the results of the random effects model
estimates store re_model

* Export Tobit results to CSV (Excel-compatible)
esttab re_model using "../results/model_7b_results.csv", replace title("Random Effects Panel Tobit Results") label se b(%9.4f) star(* 0.10 ** 0.05 *** 0.01)

log close

* Estimating a pooled Tobit model
log using "../results/model_7b_replication.log", append text
tobit prop_emne_capital_excl_col_link voice_and_acc pol_stability govt_effectiveness reg_quality rule_of_law control_of_corruption gni_per_capita broadband_per_capita mobiles_per_capita geographic_proximity, ll(0) ul(100)
